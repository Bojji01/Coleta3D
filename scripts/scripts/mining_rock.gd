...
@onready var cooperoresc = preload("res://Scene_Obj/cooper_ore.tscn")
...
func mined(): #destroi o mina e spawna o carvao
	...
	get_tree().get_current_scene().add_child(ore)
	ore.global_transform.origin = self.global_transform.origin
	ore.apply_impulse(Vector3.UP * 3)
	self.queue_free()
